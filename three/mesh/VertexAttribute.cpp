...
template<>
VertexAttribute VertexAttribute::create<glm::vec2>(int location, int offset, int stride) {
    assert(location >= 0);
    return create<glm::vec2::value_type>(location, glm::vec2::length(), offset, stride);
}

template<>
VertexAttribute VertexAttribute::create<glm::vec3>(int location, int offset, int stride) {
    assert(location >= 0);
    return create<glm::vec3::value_type>(location, glm::vec3::length(), offset, stride);
}
